...
class TestMarketDataServiceIntegration:
    ...
    def test_btc_correlation_successful_integration(self):
        ...
        # The service will call get_klines 3 times for ETH, and once for BTC inside the correlation calc.
        self.mock_api_client.get_klines.side_effect = [eth_data, eth_data, eth_data, btc_data]
    ...
    def test_btc_correlation_failure_raises_error(self):
        """Test that a failure in BTC correlation calculation raises an exception."""
        eth_data = self._create_valid_klines_data(50, 3000)
        
        # Mock 3 successful calls for ETH, and a failing call for BTC
        from src.infrastructure.exceptions import APIConnectionError
        self.mock_api_client.get_klines.side_effect = [
            eth_data,
            eth_data,
            eth_data,
            APIConnectionError("BTC API failed")
        ]
        
        # The service should fail fast when a sub-operation like correlation fails.
        with pytest.raises(APIConnectionError) as exc_info:
            self.service.get_market_data("ETHUSDT", trace_id="test_trace")
            
        assert "BTC API failed" in str(exc_info.value)
    ...
    def test_enhanced_context_full_integration(self):
        ...
        btc_data = self._create_valid_klines_data(180, 67000)
        self.mock_api_client.get_klines.return_value = btc_data
    ...
    def test_enhanced_context_edge_cases_integration(self):
        ...
        self.mock_api_client.get_klines.return_value = constant_data
        ...
        assert result.rsi_14 == Decimal('50.0')
        assert result.volume_profile in ["high", "normal", "low"]
    ...
    def test_comprehensive_validation_success_integration(self):
        ...
        self.mock_api_client.get_klines.return_value = valid_data
    ...
    def test_comprehensive_validation_failure_integration(self):
        ...
        invalid_data = [
            [1640995200000, "50000", "49000", "51000", "50500", "1000.0",
             1640995259999, "50250000.0", 100, "500.0", "25125000.0", "0"]
            for i in range(50)
        ]
        self.mock_api_client.get_klines.return_value = invalid_data
        
        from src.infrastructure.exceptions import DataFrameValidationError
    ...
    def test_cross_correlation_analysis_integration(self):
        ...
        self.mock_api_client.get_klines.return_value = base_data
        ...
        assert len(result.daily_candles.columns) == 6
    ...
    def test_caching_system_basic_integration(self):
        ...
        first_call_count = self.mock_api_client.get_klines.call_count
        
        # Second call should ideally use a cache, but for now we expect another API call
        result2 = self.service.get_market_data("BTCUSDT", trace_id="test_trace")
        second_call_count = self.mock_api_client.get_klines.call_count
        
        # For now, we expect the API to be called again. Caching is not yet implemented.
    ...
    def test_data_freshness_comprehensive_integration(self):
        ...
        self.mock_api_client.get_klines.return_value = fresh_data
    ...
    def test_full_end_to_end_integration(self):
        ...
        self.mock_api_client.get_klines.return_value = btc_main_data
        ...
        # BTC Correlation should be None for BTCUSDT
